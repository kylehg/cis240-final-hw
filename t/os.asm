...
;;;   none
	...
	.CODE
TRAP_PUTS
;;; r4 = address of ascii display status register
;;; r5 = address of ascii display data register
	LC R4, OS_ADSR_ADDR
	LC R5, OS_ADDR_ADDR

TRAP_PUTS_LOOP
	LDR R1, R0, #0		; Load next character into R1
	BRz END			; If character is null, exit

	LDR R2, R4, #0		; Wait until display is ready
	BRzp TRAP_PUTS_LOOP

	STR R1, R5, #0		; Write output to display
	
	ADD R0, R0, #1		; Advance to next character to read
...
;;;        non-0 (keyboard character)
	...
	.CODE
TRAP_GET_EVENT
;;; r0 = address of keyboard status register
;;; r1 = address of timer status register
;;; r2 = address of keyboard data register
	LC R0, OS_KBSR_ADDR
	LC R1, OS_TSR_ADDR
	LC R2, OS_KBDR_ADDR

	LDR R5, R1, #0		; Check timer
	BRn TRAP_GET_EVENT_TIME
	LDR R5, R0, #0		; If no keyboard repeat, otherwise fall through
	BRzp TRAP_GET_EVENT
TRAP_GET_EVENT_KBD
	LDR R5, R2, #0		; Read keyboard character from memory
	RTI			; Exit
TRAP_GET_EVENT_TIME
	CONST R5, #0		; Set r5 = 0 and exit
	RTI
...
;;; r5 = row stop
;;; r6 = address_stop 
	VIDEO_START .CONST 0xC000
	LC R4, VIDEO_START
	ADD R4, R4, R0		; col = col + 0xC000
	ADD R5, R1, #8		; row_stop = row + 8
TRAP_DB_ROW_LOOP
	SLL R3, R1, 7		; pixel_addr = 128 * row
	ADD R3, R4, R3		; pixel_addr += col + 0xC000
	ADD R6, R3, #8		; pixel_addr_stop = pixel_addr + 8 (end address for column loop)
TRAP_DB_COL_LOOP
	STR R2, R3, #0		; memory[pixel_addr + 0] = color
	ADD R3, R3, #1		; pixel_addr += 1
	CMP R6, R3		; pixel_addr_stop - addr
	BRp TRAP_DB_COL_LOOP	; Jump to beginning of col loop if pixel_addr_stop - pixel_addr > 0
TRAP_DB_COL_LOOP_END
	ADD R1, R1, #1		; row = row + 1
	CMP R1, R5		; row - row_stop
	BRn TRAP_DB_ROW_LOOP	; Jump to beginning of row loop if row - row_stop < 0	
TRAP_DB_ROW_LOOP_END
	RTI